 ...
 */
class PedidoEntradaForm extends TPage
{
    protected $form;
    private static $database = 'erp';
    private static $activeRecord = 'Pedido';
    private static $primaryKey = 'id';
    private static $formName = 'form_PedidoEntrada';
    private $showMethods = ['onEdit', 'onSave', 'onDelete'];
    private static $formTitle = '<i class="fas fa-clipboard-list fa-fw nav-icon"></i> Pedido de Entrada';
    ...
    function __construct($param)
    {
        ...
        $row1 = $this->form->addFields(
            [ new TLabel('Fornecedor', 'red'),      $pessoa_id          ],
        ...
        $panel = new TPanelGroup('<i class="fas fa-clipboard-list fa-fw nav-icon"></i> Itens Pedido de Entrada');
    ...
    public static function onDeleteItem($param = null)
    {
        if (isset($param['delete']) && $param['delete'] == 1) {
            try {
                ...
                new TMessage('info', $mensagem_success, new TAction(['PedidoEntradaForm', 'onEdit'],['key'=>$pedido_id]), 'Sucesso');
            } catch (Exception $e) {
            ...
            $action = new TAction(['PedidoEntradaForm', 'onDeleteItem']);
            $param['delete'] = 1;
            $param['static'] = 1;
        ...
        try
        {
           ...
           if(isset($param['quantidade']) && $param['quantidade'] == '0')
           {
            ...
            TScript::create("window.setTimeout(function(){
                __adianti_load_page('index.php?class=PedidoEntradaForm&method=onEdit&register_state=false&key={$object->id}');
            },200);");
    ...
    public static function onChangeProduto($param = null)
    {
        try
        {
            ...
            if(isset($param['produto_id']) && $param['produto_id'] != '')
            {
                ...
                $object->valor         = _formata_numero($produto->preco_custo);
            }
            else
            {
                $object->valor         = '0,00';
        ...
        try
        {
            if (isset($param['key']))
            {
                ...
                if ($this::isMobile())
                {
                    ...
                    $this->dropdown_acoes->addAction('Voltar', new TAction(['PedidoEntradaList', 'onReload']), 'fa:arrow-left black');
                }else{
                    ...
                    $this->form->addHeaderAction('Voltar', new TAction(['PedidoEntradaList', 'onReload']), 'fa:arrow-left black');
                }
            }
            else
            {
    ...
    public function onDelete($param = null)
    {
        if (isset($param['delete']) && $param['delete'] == 1) {
            try {
                ...
                new TMessage('info', 'Registro excluido com sucesso!', new TAction(['PedidoEntradaList', 'onReload']), 'Sucesso');
            } catch (Exception $e) {
