...
class RazaoEstoqueList extends TPage
{
    ...
    private static $activeRecord = 'RazaoEstoque';
    private static $primaryKey = 'id';
    private static $formName = 'form_search_RazaoEstoque';
    private $showMethods = ['onReload', 'onSearch', 'onRefresh', 'onClearFilters'];
    private static $formTitle = '<i class="fas fa-cube fa-fw nav-icon"></i> Razão do Estoque';
    private $limit = 10;
    ...
    public function __construct()
    {
        ...
        $produto_id  = new TDBCombo('produto_id', 'erp', 'Produto', 'id', '{id} - {nome} - {codigo_barras}', 'nome asc' );
        ...
        $column_pedido_id = new TDataGridColumn('pedido_id', 'Pedido', 'center');
        $column_observacao = new TDataGridColumn('observacao', 'Observação', 'left');
        $column_estoque_anterior = new TDataGridColumn('estoque_anterior', 'Anterior', 'center');
        $column_total_quantidade = new TDataGridColumn('total_quantidade', 'Quantidade', 'center');
        $column_estoque_atual = new TDataGridColumn('estoque_atual', 'Atual', 'center');
        $column_data_hora_inclusao = new TDataGridColumn('data_hora_inclusao', 'Data', 'center');
        ...
        $this->datagrid->addColumn( $column_estoque_anterior );
        $this->datagrid->addColumn( $column_total_quantidade );
        $this->datagrid->addColumn( $column_estoque_atual );
        $this->datagrid->addColumn( $column_data_hora_inclusao );
        $this->datagrid->addColumn( $column_observacao );

        $column_data_hora_inclusao->setTransformer( function($value, $object, $row) {
            return _set_format_date($value);
        });

        $column_produto_id->setAction(new TAction([$this, 'onReload']), ['order' => 'produto_id']);
