...
class CidadeList extends TPage
{
    ...
    private static $formName = 'form_search_Cidade';
    private $showMethods = ['onReload', 'onSearch', 'onRefresh', 'onClearFilters'];
    private static $formTitle = '<i class="fas fa-map-marked-alt fa-fw nav-icon"></i> Cidades';
    private $limit = 10;
    ...
    public function __construct()
    {
        ...
        $sigla = new TDBUniqueSearch('sigla', 'erp', 'Estado', 'id', 'sigla', 'sigla asc');
        $sigla->setSize('100%');
        $sigla->setMinLength(1);
        ...
        $column_nome        = new TDataGridColumn('nome', 'Cidade', 'left', '70%');
        $column_sigla       = new TDataGridColumn('{estado->sigla}', 'Estado', 'center', '10%');
        ...
        $column_sigla->setAction(new TAction([$this, 'onReload']), ['order' => 'estado->sigla']);
        $column_codigo_ibge->setAction(new TAction([$this, 'onReload']), ['order' => 'codigo_ibge']);

        $action_edit   = new TDataGridAction(['CidadeForm', 'onEdit'],   ['key' => '{id}'] );
        ...
        // $button_novo->setAction(new TAction(['CidadeForm', 'onEdit']), "Cadastrar");
    ...
    public function onSearch($param = null)
    {
        ...
        if (isset($data->sigla) AND ( (is_scalar($data->sigla) AND $data->sigla !== '') OR (is_array($data->sigla) AND (!empty($data->sigla)) )) )
        {
            ...
            $filters[] = new TFilter('estado_id', '=', "{$data->sigla}");// create the filter
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            if (empty($param['order']))
            {
                ...
                $order = $param['order'];
                if ($param['order'] == 'estado->sigla') {
                    $param['order'] = '(select sigla from estado where id = estado_id)';
                }
            }
            ...
            if(isset($param['page'])){
                TSession::setValue('pageAtual',$param['page']);
                TSession::setValue('offsetAtual',$param['offset']);
            }

            $param['page']   = TSession::getValue('pageAtual');
            $param['offset'] = TSession::getValue('offsetAtual');
