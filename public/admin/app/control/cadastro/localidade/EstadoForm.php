...
class EstadoForm extends TPage
{
    ...
    private static $formTitle = '<i class="fas fa-map-marker-alt fa-fw nav-icon"></i> Estado';
    ...
    function __construct()
    {
        ...
        $this->form->addHeaderAction('Voltar', new TAction(['EstadoList', 'onReload']), 'fa:arrow-left black');
        // if ($this::isMobile())
        // {
    ...
    public function onEdit( $param )
    {
        try
        {
            if (isset($param['key']))
            {
                ...
                //     $this->dropdown_acoes->addAction('Voltar', new TAction(['EstadoList', 'onReload']), 'fa:arrow-left black');
                // }else{
    ...
    public function onDelete($param = null)
    {
        if (isset($param['delete']) && $param['delete'] == 1) {
            try {
                ...
                new TMessage('info', 'Registro excluido com sucesso!', new TAction(['EstadoList', 'onReload']), 'Sucesso');
            } catch (Exception $e) {
