 ...
 */
class GrupoPessoaForm extends TPage
{
    protected $form;
    private static $database = 'erp';
    private static $activeRecord = 'GrupoPessoa';
    private static $primaryKey = 'id';
    private static $formName = 'form_GrupoPessoa';
    private $showMethods = ['onEdit', 'onSave', 'onDelete'];
    private static $formTitle = '<i class="fas fa-users fa-fw nav-icon"></i> Grupo Pessoa';
        ...
        try
        {
            if (isset($param['key']))
            {
                ...
                if ($this::isMobile())
                {
                    ...
                    $this->dropdown_acoes->addAction('Voltar', new TAction(['GrupoPessoaList', 'onReload']), 'fa:arrow-left black');
                }else{
                    ...
                    $this->form->addHeaderAction('Voltar', new TAction(['GrupoPessoaList', 'onReload']), 'fa:arrow-left black');
                }
            }
            else
            {
    ...
    public function onDelete($param = null)
    {
        if (isset($param['delete']) && $param['delete'] == 1) {
            try {
                ...
                new TMessage('info', 'Registro excluido com sucesso!', new TAction(['GrupoPessoaList', 'onReload']), 'Sucesso');
            } catch (Exception $e) {
