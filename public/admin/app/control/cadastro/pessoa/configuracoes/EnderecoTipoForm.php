 ...
 */
class EnderecoTipoForm extends TPage
{
    protected $form;
    private static $database = 'erp';
    private static $activeRecord = 'EnderecoTipo';
    private static $primaryKey = 'id';
    private static $formName = 'form_EnderecoTipo';
    private $showMethods = ['onEdit', 'onSave', 'onDelete'];
    private static $formTitle = '<i class="far fa-check-square fa-fw nav-icon"></i> Tipo de Endereço';
        ...
        try
        {
            if (isset($param['key']))
            {
                ...
                if ($this::isMobile())
                {
                    ...
                    $this->dropdown_acoes->addAction('Voltar', new TAction(['EnderecoTipoList', 'onReload']), 'fa:arrow-left black');
                }else{
                    ...
                    $this->form->addHeaderAction('Voltar', new TAction(['EnderecoTipoList', 'onReload']), 'fa:arrow-left black');
                }
            }
            else
            {
    ...
    public function onDelete($param = null)
    {
        if (isset($param['delete']) && $param['delete'] == 1) {
            try {
                ...
                new TMessage('info', 'Registro excluido com sucesso!', new TAction(['EnderecoTipoList', 'onReload']), 'Sucesso');
            } catch (Exception $e) {
